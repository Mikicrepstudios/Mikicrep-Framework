...
void PostWindowLogic(core::MF_Window &window) {
  ...
  if (currentTime - window.fpsAdv.lastTime >=
      1000) // Check if one second has passed
  {
    window.fpsAdv.realFPS = (float)window.fpsAdv.frameCount /
                            ((currentTime - window.fpsAdv.lastTime) / 1000.0f);
