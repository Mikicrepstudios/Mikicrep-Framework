...
bool InitWindow(core::MF_Window &window, std::string title, int width,
  ...
  if (!(IMG_Init(IMG_INIT_PNG) & IMG_INIT_PNG)) {
    std::cerr << "[MF] SDL_image could not initialize! SDL_image Error: "
              << IMG_GetError() << '\n';
    TTF_Quit();
    SDL_Quit();
    return false;
  ...
  if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 2048) < 0) {
    std::cerr << "[MF] SDL_mixer could not initialize! SDL_mixer Error: "
              << Mix_GetError() << '\n';
    IMG_Quit();
    TTF_Quit();
    SDL_Quit();
  ...
  if (!window.window) {
    std::cerr << "[MF] Window could not be created! SDL Error: "
              << SDL_GetError() << '\n';
  ...
  SDL_SetRenderDrawBlendMode(window.renderer,
                             SDL_BLENDMODE_BLEND); // Transparency

  window.currentTick = SDL_GetTicks64();
